...
if __name__ == "__main__":
    ...
    metabolites_to_exclude = set(
        map(
            lambda t: t[0],
            sorted(
                metabolite_counts.items(), key=lambda i: i[1], reverse=True
    ...
    for tf, tf_target_rxns in tqdm(
        ...
        non_targeted_rxns = [  # Really inefficient, but not rate limiting
            r
            for r in model_reactions_assoc_with_genes
            if r not in tf_target_rxns
        ]
        ...
        non_targeted_rxn_closeness = closeness_centrality_series[
            non_targeted_rxns
        ]
        ...
        targeted_rxn_betweenness = betweenness_centrality_series[
            tf_target_rxns
        ]
        ...
        tf_centrality_results_df.loc[tf, "betweenness p-value"] = (
            betweenness_pval
        )
    ...
    tf_centrality_results_df["betweenness bootstrap adj p-value"] = (
        fdr_with_nan(tf_centrality_results_df["betweenness bootstrap p-value"])
    )
