...
if __name__ == "__main__":
    ...
    logger.info(
        "Evaluating Eigenvector centrality significance for TF targets"
    )
    for tf, tf_target_series in tf_target_df.items():
        ...
        non_targeted_centrality = eigenvector_centrality.drop(
            target_reaction_list
        )
        if (
    ...
    results_df.to_csv(
        RESULTS_PATH / "mutual_information_tf_target_centrality.csv"
    )
