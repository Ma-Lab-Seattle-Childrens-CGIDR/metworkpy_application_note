...
if __name__ == "__main__":
    ...
    for rxn in BASE_MODEL.reactions:
        ...
        if subsys not in subsystems_to_ignore:
            divergence_rxn_group_dict["subsystem__Whole Metabolism"].append(
                rxn.id
            )
    ...
    for metabolite in BASE_MODEL.metabolites:
        ...
        divergence_rxn_group_dict[
            f"metabolite_consumption__{metabolite.id}"
        ] = sorted(
            set(
    ...
    for sample in tqdm(
        ...
        sample_flux_dist = pd.read_parquet(
            FLUX_SAMPLES_PATH / f"{sample}.parquet"
        )
        ...
        assert isinstance(div_res, pd.Series)
