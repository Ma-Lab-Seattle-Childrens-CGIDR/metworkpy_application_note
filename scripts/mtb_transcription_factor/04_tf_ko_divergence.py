...
if __name__ == "__main__":
    ...
    metabolite_synthesis_rxn_network_df = (
        metabolite_synthesis_rxn_network_df.drop(
    ...
    common_genes = sorted(model_gene_set & vi_genes)
    ...
    biomass_ko_div = ko_divergence_df.loc[common_genes, :][
        "BIOMASS__2__reaction"
    ]
    ...
    fba_ess_series = (
        fba_ko_series
        <= CONFIG["mtb_tf"]["ko_divergence"]["fba_ess_threshold"]
        * optimal_growth
    )[common_genes]
    ...
    biomass_ko_div_stat_res[
        "FBA Predicted Essentiality-KO Divergence AUC ROC"
    ] = fba_ko_div_auc
    biomass_ko_div_stat_res[
    ...
    non_essential_fba_growth_series = fba_ko_series[
        ~(tnseq_ess == "Essential")
    ]
    ...
    biomass_ko_div_stat_res["Essentiality-FBA Predicted Growth U1"] = (
        fba_growth_u1
    )
    biomass_ko_div_stat_res["Essentiality-FBA Predicted Growth U2"] = (
        fba_growth_u2
    )
    ...
    for tf, target_list in tf_target_dict.items():
        ...
        for metabolite, ko_divergence_series in ko_divergence_df.items():
            ...
            rho = u1 / (
                len(targeted_divergence) * len(non_targeted_divergence)
            )
